...
export const Navbar = ({ activeTab, setActiveTab, sidebarOpen, setSidebarOpen }) => {
  ...
  const { coords, isLoading: isLocLoading, openLocationSelector } = useLocation();
  ...
  return (
    <header className="sticky top-0 z-40 w-full glass-panel border-b border-slate-800/80 bg-[#090d16]/90 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        {/* Center: Interactive Live Location Indicator */}
        <button
          onClick={openLocationSelector}
          title="Click to Switch State, District or City (e.g. Karur, Tamil Nadu)"
          className="hidden md:flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-900/90 border border-slate-700/80 hover:border-rose-500/50 hover:bg-slate-800/80 text-xs text-slate-300 transition group shadow-sm cursor-pointer"
        >
          ...
          <span className="truncate max-w-[200px] text-slate-200 font-semibold group-hover:text-rose-300 transition" title={coords.address}>
            {isLocLoading ? 'Locating GPS...' : `${coords.city || 'Karur'}${coords.state ? ', ' + coords.state : ''}`}
          </span>
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-rose-500/20 text-rose-300 border border-rose-500/30 font-bold group-hover:bg-rose-500 group-hover:text-white transition">
            Change
          </span>
        </button>
